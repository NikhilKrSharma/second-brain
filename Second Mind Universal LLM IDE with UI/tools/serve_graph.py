...
def main() -> None:
    ...
    parser.add_argument(
        "--allow-write",
        action="store_true",
        help="Allow HTTP PUT to overwrite wiki/*.md (localhost only; do not expose to untrusted networks)",
    )
    ...
    if allow_write:
        print(
            "  *** PUT enabled for wiki/**/*.md (UTF-8, max "
            f"{MAX_PUT_BYTES} bytes). Bind: 127.0.0.1 only — do not expose to untrusted networks. ***"
        )
    if not args.no_open:
        webbrowser.open(url)
    try:
